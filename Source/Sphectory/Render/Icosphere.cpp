/* Slightly modified (specific for this project) version of an
 * Icosphere generator implementation by Song Ho Ahn:
...
std::shared_ptr<Ksm::Mesh> Icosphere::GetMesh()
{
    std::vector<Ksm::Vertex> vs;
    for (int i = 0; i < vertices.size(); i+=3) {
        vs.push_back({
            glm::vec3(vertices[i], vertices[i+1], vertices[i+2]),
            glm::vec3(1.0f, 1.0f, 1.0f),
            glm::vec3(normals[i], normals[i+1], normals[i+2]),
            glm::vec2(texCoords[i * 2/3], texCoords[(i * 2/3) + 1])
        });
    }
    return std::make_shared<Ksm::Mesh>(vs, indices);
}

Icosphere::Icosphere(float radius, int sub, bool smooth)
...
void Icosphere::addSubLineIndices(
    unsigned int i1,
    unsigned int i2,
    unsigned int i3,
    unsigned int i4,
